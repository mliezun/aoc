...
def neighbors(garden, x, y):
    for dx, dy in [(1, 0), (-1, 0), (0, 1), (0, -1)]:
        if 0 <= x+dx < len(garden) and 0 <= y+dy < len(garden[0]) and garden[x+dx][y+dy] in ('.', 'S'):
            yield (x+dx, y+dy)
            
def walk_garden(garden, steps):
    current_pos = next((x, y) for x, row in enumerate(garden) for y, c in enumerate(row) if c == "S")
