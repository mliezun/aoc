        ...
        for i in range(len(self.layout)):
            for j in range(len(self.layout[i])):
                ...
                for dx, dy in DIRS_MAP.keys():
                    ...
                    if (
                        0 <= new_x < len(self.layout)
                        and 0 <= new_y < len(self.layout[0])
                        and self.layout[new_x][new_y] != " "
                    ):
...
def compute_results(codes: list[str]):
    ...
    for code in codes:
        ...
        for numeric_keys in numeric_options:
            directional_1_options = compute_all_possible_moves_for_keys(
                numeric_keys, directional_1
            )
            for directional_1_keys in directional_1_options:
                directional_2_options = compute_all_possible_moves_for_keys(
                    directional_1_keys, directional_2
                )
                for directional_2_keys in directional_2_options:
                    directional_2_results.append(
                        (numeric_keys, directional_1_keys, directional_2_keys)
                    )
